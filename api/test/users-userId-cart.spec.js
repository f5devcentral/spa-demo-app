...
describe("GET /api/users/{userId}/cart", function () {
  ...
  beforeEach(() => {
    ...
    stubFindOneUser = sinon.stub().resolves(
      aUser
    )
...
describe("POST /api/users/{userId}/cart", function () {
  ...
  beforeEach(() => {
    ...
    stubFindOneProduct = sinon.stub().resolves(
      aProduct
    )
  ...
  it("Should add a product to the user cart", async function () {
    ...
    expect(response.status).to.equal(200)
    expect(response.body.length).to.equal(2)
    expect(stubFindOneProduct.callCount).to.equal(1)
    expect(stubFindOneUser.callCount).to.equal(1)
    expect(stubFind.callCount).to.equal(1)
    expect(stubUpdateOne.callCount).to.equal(1)
    expect(mockConnectionStub.callCount).to.equal(2)
    expect(mockInstanceStub.close.called).to.be.true
  })

  it("Should emit a 404 error if the user does not exist in the database", async function () {
    stubFindOneUser = sinon.stub().resolves(null)
    mockConnectionStub = stub(MongoClient, "connect").resolves(Promise.resolve(mockInstanceStub))
  ...
  it("Should emit a 404 error if the product being added does not exist in the database", async function () {
    stubFindOneProduct = sinon.stub().resolves(null)
    mockConnectionStub = stub(MongoClient, "connect").resolves(Promise.resolve(mockInstanceStub))
    ...
    expect(response.headers["content-type"]).to.match(/json/)
    expect(response.status).to.equal(404)
    expect(response.body).to.deep.equal({ error: "Could not find the product!" })
    expect(stubFindOneProduct.callCount).to.equal(1)
    expect(stubFindOneUser.callCount).to.equal(0)
